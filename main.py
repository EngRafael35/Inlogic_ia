...
if __name__ == '__main__':
    ...
    sistema = None
    try:
        # 1. Manager como contexto para garantir limpeza de recursos
        with multiprocessing.Manager() as manager:
            print(f"{Style.BRIGHT}{Fore.GREEN}Iniciando Ecossistema Cognitivo InLogic...{Style.RESET_ALL}")
            
            # 2. Cria o sistema principal
            sistema = SistemaPrincipal(manager)
            
            # 3. Inicia todos os componentes de forma organizada
            sistema.iniciar_subsistemas()
            
            log('SUCCESS', 'MAIN', "Sistema principal totalmente operacional. Pressione Ctrl+C para encerrar.")
            
            # 4. Mantém o processo principal vivo, enquanto a flag 'running' for True
            #    As threads em background (API, Distribuidor) fazem o trabalho.
            while sistema.running:
                time.sleep(1) # Laço de espera de baixa carga

    except KeyboardInterrupt:
        log('INFO', 'MAIN', "Sinal de interrupção (Ctrl+C) recebido. Encerrando sistema...")
    except Exception as e:
        log('FATAL', 'MAIN', "Ocorreu um erro fatal e irrecuperável.", details={'erro': str(e)})
        import traceback
        traceback.print_exc()
    finally:
        # 5. O bloco finally GARANTE que a parada limpa seja chamada, não importa como o programa termine
        if sistema:
            sistema.parar()
